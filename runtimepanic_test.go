...
func TestRuntime_panic(t *testing.T) {
	tests := []struct {
		...
		{name: "nil_pointer_dereference2",
			f:    nil_pointer_dereference2,
			want: "runtime error: invalid memory address or nil pointer dereference",
		},
		{name: "cannot_convert_slice",
			f:    cannot_convert_slice,
			want: "runtime error: cannot convert slice with length 2 to pointer to array with length 4",
		},
	}
